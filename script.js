...
document.getElementById('cdsHotelCard').innerHTML=`
<div class="cds-hotel-orb">
<svg class="s16" viewBox="0 0 24 24" style="width:22px;height:22px;color:#fff;"><path d="M3 21h18M5 21V7l7-4 7 4v14M9 21v-6h6v6"/></svg>
</div>
<div class="cds-hotel-main">
<div class="cds-hotel-name">${hotel.name||'اقامتگاه'}</div>
<div class="cds-hotel-meta">
${starStr?`<span class="cds-hotel-stars">${starStr}</span>`:''}
${hotel.dist?`<span class="cds-hotel-dist">${hotel.dist}</span>`:''}
</div>
</div>`;
document.getElementById('cdsDays').innerHTML=days.map((d,i)=>`
<div class="cds-day" style="animation-delay:${i*.05}s">
<div class="cds-day-num">${(i+1).toLocaleString('fa-IR')}</div>
<div class="cds-day-txt">${d}</div>
</div>`).join('');
...
document.getElementById('gcalCity')?.addEventListener('change',e=>{filterCity=e.target.value;filterCaravans();});
