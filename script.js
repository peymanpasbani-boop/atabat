...
const featured=activeOrigins.slice(0,4);
const overflow=activeOrigins.slice(4);
...
function hiRenderHiCal() {
  ...
  if(labelEl) labelEl.textContent = HI_J_MONTHS[m] + ' ' + HI_TODAY.y.toLocaleString('fa-IR');
  ...
  for(let d=1;d<=total;d++) {
    ...
    const clickAttr = isPast ? '' : `onclick="hiCalPick(${m},${d})"`;
...
function hiCalPick(m, d) {
  ...
  if(hiCal.target==='checkin') {
    hiCal.checkin = {m,d};
    hiCal.checkout = null;
    const v = d.toLocaleString('fa-IR')+' '+HI_J_MONTHS[m]+'ماه '+HI_TODAY.y.toLocaleString('fa-IR');
...
let flSelD = null; /* selected day number within current jalali month */
...
function flRenderCal(){
  ...
  for(let d=1; d<=FL_DAYS_IN_MONTH; d++){
    const dow = flDowName(FL_YEAR,FL_MONTH,d);
    const isPast = jIsPastDay(FL_YEAR,FL_MONTH,d);
    const isToday = today.y===FL_YEAR&&today.m===FL_MONTH&&today.d===d;
    ...
    if(isPast) cls+=' fl-daycard--disabled';

    const priceHtml = isPast
      ? `<span class="fl-dc-noflight">—</span>`
      : (price!=null
        ? `<span class="fl-dc-price">${toFa(price.toFixed(1))} م</span>`
        : `<span class="fl-dc-noflight">—</span>`);

    const clickAttr = isPast ? '' : `onclick="flSelectDay(${d})"`;
    html+=`<button class="${cls}" data-day="${d}" ${clickAttr} ${isPast?'disabled':''} aria-label="${dow} ${toFa(d)} ${jMonths[FL_MONTH-1]}">
      <span class="fl-dc-dow">${dow}</span>
      <span class="fl-dc-date">${toFa(d)} ${jMonths[FL_MONTH-1]}</span>
      ${priceHtml}
    </button>`;
  }
...
window.flSelectDay=function(d){
  if(jIsPastDay(FL_YEAR,FL_MONTH,d)) return;
  flSelD=d;
  flRenderCal();
  const lbl=document.getElementById('fl-date-label-text');
  const dow = flDowName(FL_YEAR,FL_MONTH,d);
  if(lbl) lbl.textContent=`${dow} ${toFa(d)} ${jMonths[FL_MONTH-1]} ${toFa(FL_YEAR)}`;
  flRenderList();
};
