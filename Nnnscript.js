...
updatePreview();
// ── Hide-on-scroll ad widget ──────────────────────────────────────
(function(){
  const ad=document.getElementById('adSlider');
  if(!ad)return;
  const panel=ad.closest('.panel')||null;
  const scroller=panel||window;
  let lastY=0,ticking=false;
  const THRESHOLD=80;

  function getY(){return scroller===window?window.scrollY:(scroller.scrollTop||0);}
...
// ── Nav scrolled class + smooth scroll fix for panel-home ────────
(function(){
  const home=document.getElementById('panel-home');
  const nav=home&&home.querySelector('.h-nav');
  if(!home||!nav)return;
  home.addEventListener('scroll',()=>{
    nav.classList.toggle('scrolled',home.scrollTop>8);
  },{passive:true});

  // Fix: prevent cards from blocking scroll (touch-action)
  home.querySelectorAll('.h-cards .cc').forEach(card=>{
