...
function flUpdateHeroRoute(){
  ...
  const subEl=document.querySelector('.fl-hero-sub');
  if(flDirection==='to'){
    if(oc) oc.textContent=flOriginCode;
    if(on) on.textContent=flOrigin;
    if(dc) dc.textContent='NJF';
    if(dn) dn.textContent='نجف اشرف';
    if(subEl) subEl.innerHTML=`<span class="fl-live-dot" aria-hidden="true"></span>مستقیم &nbsp;·&nbsp; بدون توقف &nbsp;·&nbsp; رفت به نجف`;
  } else {
    if(oc) oc.textContent='NJF';
    if(on) on.textContent='نجف اشرف';
    if(dc) dc.textContent=flOriginCode;
    if(dn) dn.textContent=flOrigin;
    if(subEl) subEl.innerHTML=`<span class="fl-live-dot" aria-hidden="true" style="background:#e05c3a;box-shadow:0 0 0 0 rgba(224,92,58,.6);"></span>مستقیم &nbsp;·&nbsp; بدون توقف &nbsp;·&nbsp; برگشت از نجف`;
  }
}
