...
/* ── تب سراسری: ساخت مسیر / رزرو مستقیم هتل ──────────
   این تب یک «راهنمای ناوبری» است، نه یک سوییچ حالت دائمی:
   با زدن «رزرو مستقیم هتل» فقط به همان باکس جستجوی هتل که
   پایین‌تر در صفحه قرار دارد اسکرول می‌شویم و هایلایتش می‌کنیم،
   سپس تب به‌صورت خودکار به حالت پیش‌فرض برمی‌گردد تا کاربر
   گمراه نشود که گویی صفحه یا فرمی عوض شده است ── */
function gtSelect(btn) {
  const tabs = document.getElementById('gtTabs');
  if (!tabs || !btn) return;
  const mode = btn.dataset.mode;
  tabs.querySelectorAll('.gt-tab').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
  tabs.dataset.mode = mode;

  if (mode === 'hotel') {
    const target = document.getElementById('hseCombo');
    if (target) {
      target.scrollIntoView({ behavior: 'smooth', block: 'center' });
      target.classList.remove('gt-pulse');
      requestAnimationFrame(() => target.classList.add('gt-pulse'));
      setTimeout(() => target.classList.remove('gt-pulse'), 2400);
    }
    // برگشت خودکار به حالت پیش‌فرض پس از مکث کوتاه؛ این تب فقط یک اشاره‌گر است
    setTimeout(() => {
      const routeBtn = tabs.querySelector('.gt-tab[data-mode="route"]');
      if (routeBtn) {
        tabs.querySelectorAll('.gt-tab').forEach(b => b.classList.remove('active'));
        routeBtn.classList.add('active');
        tabs.dataset.mode = 'route';
      }
    }, 900);
  }
}
...
function hiOpenSheet(idx) {
  ...
  /* اگر کاربر از جستجوی صفحه اصلی آمده، همان تاریخ‌ها را (در صورت باز بودن ظرفیت
     برای این هتل خاص) پیش‌فرض قرار می‌دهیم؛ در غیر این صورت فرم خام باز می‌شود */
  const s = window._hseSearch;
  const ciInp = document.getElementById('hiCheckin');
  const coInp = document.getElementById('hiCheckout');
  if (s && !hiRangeHasClosedDate(h, s.checkin, s.checkout)) {
    hiCal.checkin = { ...s.checkin };
    hiCal.checkout = { ...s.checkout };
    if (ciInp) ciInp.value = s.checkin.d.toLocaleString('fa-IR') + ' ' + HI_J_MONTHS[s.checkin.m] + 'ماه ' + toFarsiNum(HI_TODAY.y);
    if (coInp) coInp.value = s.checkout.d.toLocaleString('fa-IR') + ' ' + HI_J_MONTHS[s.checkout.m] + 'ماه ' + toFarsiNum(HI_TODAY.y);
  } else {
    hiCal.checkin = null;
    hiCal.checkout = null;
    if (ciInp) ciInp.value = '';
    if (coInp) coInp.value = '';
    if (s) showToast('این هتل در تاریخ جستجوی شما ظرفیت ندارد؛ تاریخ دیگری انتخاب کنید');
  }
  hiRenderNightsSummary();
  hiRenderRoomtypeGrid();
  hiUpdateSheetTotal();
...
function generateFlights(origin, jy, jm, jd, route){
  ...
  const routeKey = window._avanDepRouteKey; /* مسیر فعال پنل پرواز نجف، مثلاً "تهران→نجف" */
