  ...
  grid.innerHTML = list.map((h,i) => {
    ...
    return `<div class="hi-card" style="animation-delay:${i*70}ms" onclick="hiOpenSheet(${i})">
      <div class="hi-card-top">
        ...
        <div class="hi-card-grade-badge" style="color:#FFF8EE;background:rgba(255,255,255,.22);border:1px solid rgba(255,255,255,.4)">${g.label}</div>
      </div>
...
function hiShowReview() {
  ...
  document.getElementById('hiReviewBody').innerHTML = `
    ...
    <div>🛏 تعداد اتاق:<b>${b.rooms.toLocaleString('fa-IR')}</b> &nbsp;|&nbsp; 👥 تعداد نفر:<b>${b.guests.toLocaleString('fa-IR')}</b></div>
...
async function hiSendToTelegram() {
  ...
  msg += `🛏 اتاق:${b.rooms} | 👥 نفرات:${b.guests}\n`;
  msg += `👤 سرپرست:${b.name}\n`;
  msg += `📱 موبایل:${b.phone}\n`;
