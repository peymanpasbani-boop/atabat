...
`نجف ${c.najaf}شب`,
`از ${c.origin}`,
...(c.kazTransit?[]:[`کاظمین ${c.kazNights}شب`])
],
meta:`🗓 ${c.meta||''}`,
sub:c.sub||''
}));
...
const{tag,title,grad,price,origin,transport,stops=[],hotel={},chips=[],days=[],hotels=[]}=data;
...
document.getElementById('cdsRouteBadge').innerHTML=
`<div class="cds-rm-badge">${transport==='land'?
'<svg class="s18" viewBox="0 0 24 24" style="width:14px;height:14px;"><rect x="1" y="6" width="22" height="12" rx="3"/><path d="M7 18v2M17 18v2M1 12h22"/></svg>اتوبوس VIP از مهران':
'<svg class="s18" viewBox="0 0 24 24" style="width:14px;height:14px;"><path d="M22 2 11 13M22 2 15 22l-4-9-9-4 20-7z"/></svg>پرواز مستقیم'
}</div>`;
const starCount=parseInt((hotel.stars||'').replace(/[★۱-۵]/g,s=>'★۱۲۳۴۵'.indexOf(s)))||0;
const starStr=hotel.stars||'';
// Build trip type section
const ttHotels=hotels&&hotels.length?hotels:(hotel&&hotel.name?[{city:'کربلا',...hotel}]:[]);
const ttChipTransport=chips.find(c=>c.lbl==='نوع سفر');
const ttTransportLabel=ttChipTransport?ttChipTransport.val:(transport==='land'?'زمینی':'هوایی');
const ttTransportIcon=transport==='land'
?`<svg viewBox="0 0 24 24" style="width:16px;height:16px;fill:none;stroke:currentColor;stroke-width:2;stroke-linecap:round"><rect x="1" y="6" width="22" height="12" rx="3"/><path d="M7 18v2M17 18v2M1 12h22"/></svg>`
:`<svg viewBox="0 0 24 24" style="width:16px;height:16px;fill:none;stroke:currentColor;stroke-width:2;stroke-linecap:round"><path d="M22 2 11 13M22 2 15 22l-4-9-9-4 20-7z"/></svg>`;
const borderNote=transport==='land'?'<div class="cds-tt-border">🛂 عبور از مرز مهران</div>':'';
document.getElementById('cdsTripType').innerHTML=`
<div class="cds-tt-wrap">
  <div class="cds-tt-transport">
    <div class="cds-tt-ticon">${ttTransportIcon}</div>
    <span class="cds-tt-tlabel">${ttTransportLabel}</span>
  </div>
  ${borderNote}
  <div class="cds-tt-hotels">
    ${ttHotels.map(h=>`
    <div class="cds-tt-hotel-row">
      <div class="cds-tt-hotel-city-dot"></div>
      <div class="cds-tt-hotel-info">
        <div class="cds-tt-hotel-city">${h.city||'کربلا'}</div>
        <div class="cds-tt-hotel-name">${h.name||'اقامتگاه'}${h.stars?` &nbsp;<span class="cds-tt-stars">${h.stars}</span>`:''}</div>
        ${h.dist?`<div class="cds-tt-hotel-dist">📍 ${h.dist} تا حرم</div>`:''}
      </div>
      ${(()=>{const nb=chips.find(c=>c.lbl===h.city);return nb?`<div class="cds-tt-nights">${nb.val}</div>`:'';})()}
    </div>`).join('')}
  </div>
</div>`;
...
<div class="cvb">
<div class="cvt"><div class="cvtitle">${c.title}</div><div class="cvprice"><small>هر نفر از</small>${c.price}ت</div></div>
...
if(!el)return;
const stats=[
{num:'۳',lbl:'کاروان فعال'},
{num:'۴',lbl:'شهر مبدأ'},
{num:'۲۳',lbl:'نفر ظرفیت'},
{num:'۷',lbl:'روز میانگین'},
];
el.innerHTML=stats.map((s,i)=>`<div class="cst-item" style="animation-delay:${i*.08}s">
...
const departures={
3:[{d:20,ci:0},{d:25,ci:1}],
4:[{d:1,ci:2},{d:15,ci:0}],
5:[{d:10,ci:1}],
6:[{d:5,ci:0},{d:22,ci:2}],
7:[{d:10,ci:1},{d:18,ci:0}],
};
let curMonth=3;
let selStart=null,selEnd=null,hovDay=null;
...
const deps=departures[curMonth]||[];
...
<strong>${o.lbl}</strong>
<span>تا ${toFa(d+o.days)}${mn}</span>
</button>`).join('');
optsEl.querySelectorAll('.gcal-range-opt').forEach(btn=>{
btn.addEventListener('click',()=>{
const nd=d+(+btn.dataset.days);
const maxD=dpm[m];
selEnd=nd<=maxD?{d:nd,m}:{d:nd-maxD,m:m<11?m+1:0};
...
if(selStart){
const inRange=Object.entries(departures).some(([mi,arr])=>
arr.some(dep=>{
if(dep.ci!==ci)return false;
const dm=+mi,dd=dep.d;
const afterStart=dm>selStart.m||(dm===selStart.m&&dd>=selStart.d);
const beforeEnd=!selEnd||(dm<selEnd.m||(dm===selEnd.m&&dd<=selEnd.d));
...
renderCaravans(document.getElementById('cgrid-group'),false);
