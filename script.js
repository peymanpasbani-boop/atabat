...
let flCalYear, flCalMonth;
let flSelY, flSelM, flSelD;
let flSelFlight = null;
let flAdult = 1, flChild = 0;

/* Days with flights — generate for whole month deterministically */
function getDaysWithFlights(jy,jm){
  const days={};
  const total=jDaysInMonth(jy,jm);
  for(let d=1;d<=total;d++){
    // Most days have flights, some don't
    const r=seededRandom(jy*100000+jm*1000+d*17+flOrigin.charCodeAt(0));
    if(r>0.1){ // 90% of days have flights
...
function flRenderCal(){
  ...
  const grid=document.getElementById('fl-cal-days');
  if(!lbl||!grid) return;
  lbl.textContent=`${jMonths[flCalMonth-1]} ${toFa(flCalYear)}`;

  const [ty,tm,td]=todayJalali();
  const firstDayJalali=[flCalYear,flCalMonth,1];
  const [gy,gm,gd]=jalaliToGregorian(flCalYear,flCalMonth,1);
  const jsDate=new Date(gy,gm-1,gd);
  // Saturday=0 in Jalali week, but getDay(): 0=Sun,6=Sat
  const jsDow=jsDate.getDay(); // 0=Sun
  // Jalali week starts Sat(=6 in JS)
  const startBlank=(jsDow+1)%7;
  const totalDays=jDaysInMonth(flCalYear,flCalMonth);
  const daysWithFlights=getDaysWithFlights(flCalYear,flCalMonth);

  let html='';
  for(let i=0;i<startBlank;i++) html+='<div class="fl-cal-day fl-cal-day--empty"></div>';
  for(let d=1;d<=totalDays;d++){
    const isPast=(flCalYear<ty)||(flCalYear===ty&&flCalMonth<tm)||(flCalYear===ty&&flCalMonth===tm&&d<td);
    const isToday=flCalYear===ty&&flCalMonth===tm&&d===td;
    const isSel=flSelY===flCalYear&&flSelM===flCalMonth&&flSelD===d;
    const hasFlight=!!daysWithFlights[d]&&!isPast;
    const many=daysWithFlights[d]>3;
    let cls='fl-cal-day';
    if(isPast) cls+=' fl-cal-day--past';
    if(isToday) cls+=' fl-cal-day--today';
    if(isSel) cls+=' fl-cal-day--selected';
    if(hasFlight) cls+=' fl-cal-day--has-flight';
    const dot=hasFlight?`<div class="fl-day-dot${many?' fl-day-dot--multi':''}"></div>`:'';
    html+=`<button class="${cls}" onclick="flSelectDay(${d})" aria-label="${toFa(d)} ${jMonths[flCalMonth-1]}">
      <span class="fl-day-num">${toFa(d)}</span>${dot}
    </button>`;
  }
  grid.innerHTML=html;
}

window.flCalPrev=function(){
  flCalMonth--;
  if(flCalMonth<1){flCalMonth=12;flCalYear--;}
  flRenderCal();
};
window.flCalNext=function(){
  flCalMonth++;
  if(flCalMonth>12){flCalMonth=1;flCalYear++;}
  flRenderCal();
};

window.flSelectDay=function(d){
  flSelY=flCalYear; flSelM=flCalMonth; flSelD=d;
  flRenderCal();
  // Update label
  const lbl=document.getElementById('fl-date-label-text');
  if(lbl) lbl.textContent=`${toFa(d)} ${jMonths[flCalMonth-1]} ${toFa(flCalYear)}`;
  flRenderList();
};
...
function flRenderList(){
  ...
  const flights=generateFlights(flOrigin,flSelY,flSelM,flSelD);
  if(!flights.length){
...
window.flOpenSheet=function(i, f){
  ...
  if(info){
    const totalP=(f.price*(flAdult+flChild*0.5)).toFixed(1);
...
window.flCounter=function(type,delta){
  if(type==='adult'){flAdult=Math.max(1,flAdult+delta);}
  else{flChild=Math.max(0,flChild+delta);}
  const av=document.getElementById('fl-adult-val');
  const cv=document.getElementById('fl-child-val');
  if(av) av.textContent=toFa(flAdult);
  if(cv) cv.textContent=toFa(flChild);
  flUpdateSheetTotal();
};

function flUpdateSheetTotal(){
  if(!flSelFlight) return;
  const total=(flSelFlight.price*flAdult+flSelFlight.price*0.5*flChild).toFixed(1);
  const el=document.getElementById('fl-sheet-total');
  if(el) el.textContent=`${toFa(total)} میلیون`;
}

window.flSubmitReserve=function(){
  const name=document.getElementById('fl-name')?.value?.trim();
  const phone=document.getElementById('fl-phone')?.value?.trim();
  if(!name||!phone){alert('لطفاً نام و شماره موبایل را وارد کنید.');return;}
  flCloseSheet();
  // Show toast
  const t=document.getElementById('toast');
  if(t){
    t.textContent='✦ درخواست رزرو ثبت شد — کارشناسان آوان با شما تماس می‌گیرند';
...
window.openPanel=function(id){
  if(id==='flight'){
    ...
    flCalYear=jy; flCalMonth=jm;
    // Don't preselect a date
    flSelD=null;
    setTimeout(()=>{flRenderCal();flRenderList();},80);
  }
