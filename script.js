...
const paxLabel=toFarsiNum(adultCount)+' بزرگسال'+(infantCount?' · '+toFarsiNum(infantCount)+' نوزاد':'');
...
<div class="pi-trip-price">${(data.totalPrice||0).toLocaleString('fa-IR')} تومان · ${paxLabel}${data.addons?.length?' · '+data.addons.join('، '):''}</div>
</div>`;
...
function toggleAddon(){}
/* ── باز کردن modal انتخاب تعداد زائران هنگام کلیک رزرو ── */
let _cvPaxAdult = 1;
let _cvPaxInfant = 0;
...
function cvConfirmPax() {
  ...
  openPilgrimSheet({
    ...
    adultCount: _cvPaxAdult,
    infantCount: _cvPaxInfant,
    addons: [
